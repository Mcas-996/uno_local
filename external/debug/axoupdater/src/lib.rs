...
fn installer_command(installer_path: &Utf8PathBuf) -> Cmd {
    if cfg!(windows) {
        let mut command = Cmd::new("powershell", "execute installer");
        // Don't fall over on default security-policy Windows machines which
        // require opt-in to executing PowerShell scripts. This doesn't bypass
        // proper organization-set policies.
        //
        // `-File` is required here: without it, PowerShell treats the path as
        // command text, so an installer under a path containing spaces (such
        // as `C:\Users\First Last\...`) is truncated at the first space.
        command
            .arg("-ExecutionPolicy")
            .arg("ByPass")
            .arg("-File")
        ...
        Cmd::new(installer_path.as_str(), "execute installer")
    }
}
...
mod tests {
    ...
    #[cfg(windows)]
    #[test]
    fn test_powershell_installer_path_with_spaces() {
        use camino::Utf8PathBuf;
        use tempfile::TempDir;

        use crate::installer_command;

        let tempdir = TempDir::new().unwrap();
        let installer_dir = tempdir.path().join("path with spaces");
        std::fs::create_dir(&installer_dir).unwrap();
        let installer_path = installer_dir.join("installer.ps1");
        std::fs::write(&installer_path, "Write-Output 'installer-ran'").unwrap();
        let installer_path = Utf8PathBuf::from_path_buf(installer_path).unwrap();
        ...
        assert!(String::from_utf8_lossy(&output.stdout).contains("installer-ran"));
    }
