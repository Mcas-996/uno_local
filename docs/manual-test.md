...
2. 左侧使用 `WASD`、右侧使用方向键或 `hjkl` 选择手牌；非当前玩家应能预选，但不能改变回合。
3. 使用共享的 `Enter` 出牌、`X` 摸牌、`P` 跳过；确认 `D` 在双人模式中只让左侧选牌向右移动。
4. 两侧玩家分别打出万能牌，确认只能用当前玩家的左右键选色，并用 `Enter` 确认。
5. 在文字和图像模式下确认当前玩家的选中牌预览、弃牌、长手牌滚动和回合标记正确。
