...
设置页支持单人和本地双人对局。双人模式可加入 0–3 个 AI，左侧玩家固定使用 `WASD`，右侧玩家固定使用 `hjkl`，方向键控制当前回合的真人玩家；双方共用 `Enter/X/P` 出牌、摸牌和跳过。对局中按 `F` 会让双方手牌在“全部、+、-、0/7”间循环筛选，可见牌会从 1 开始连续编号。

每位玩家的核心手牌只保留当前批次最多 200 张具体牌，其余牌只记录数量。面板显示总张数、当前页和已加载张数；真人在底部累计按 5 次下键加载下一随机页，在顶部累计按 5 次上键重新生成上一页。旧页牌面不会保存，重新访问时会再次随机生成；生成过程使用独立进度遵守真人保底以及 AI 难度的保底/排除规则，不消耗公共摸牌堆。AI 只根据当前批次决策，并在该批次出空后自动生成下一批。
